...
public class Server {
	...
	public static int port = 9999, counter = 0, totalNumPlayers = 15, playerID;
	public static String ip = "127.0.0.1", numbers, operator, numberGroup, op, clientGroup, serverGroup, answer;
	private ServerSocketConnection player1, player2, player3, player4, player5, player6, player7, player8,
			player9, player10, player11, player12, player13, player14, player15;
	private ServerSocketConnection[] ServerSocketConnectionList;
	static DataOutputStream serverOut1, serverOut2, serverOut3, serverOut4, serverOut5, serverOut6, serverOut7,
			serverOut8, serverOut9, serverOut10, serverOut11, serverOut12, serverOut13, serverOut14, serverOut15;
	private static DataOutputStream[] outputStreamList, hackerOutputStreamList, hostOutputStreamList,
			techOutputStreamList;
	public static DataInputStream clientIn1, clientIn2, clientIn3, clientIn4, clientIn5, clientIn6, clientIn7,
			clientIn8, clientIn9, clientIn10, clientIn11, clientIn12, clientIn13, clientIn14, clientIn15;
	...
	// create list to save players to nonHackerList to pass to hackers & playerList
	// for techs & hosts
	public static String[] nonHackerList, playerList, hackedPlayers, hostList, techList, probableHackers, hackerList,
			techHealed, recoveredHosts, techHealedForTurn;

	/*
	 * 
	 * USE THIS IN THE COMMAND LINE FOR NOW. IT IS MUCH EASIER TO HANDLE MULTIPLE
	 * CLIENTS THAT WAY. AT LEAST UNTIL WE GET THE GAME RUNNING. WE CAN SEND
	 * MESSAGES TO THE SERVER FROM ALL THE CLIENTS NOW!!! YEET!
	 * 
	 */
	...
	public Server()	{
		...
		try {
			...
			System.out.println("IO Exception in Server Constructor");
		}	
	}

	private class ServerSocketConnection {
		private Socket socket;
		private DataInputStream dataIn;
		private DataOutputStream dataOut;
		private int playerID;

		public ServerSocketConnection(Socket socket, int playerID) {
			this.socket = socket;
			...
			try {
				...
				System.out.println("IO Exception in ServerSocketConnection()");
			}

		}
	}
	
	public void accpetPlayerConnections(){
		playerID = 0;
		while (playerID < totalNumPlayers) {
			try {
				Socket socket = serverSocket.accept();
				ServerSocketConnection newPlayerConnection = new ServerSocketConnection(socket, playerID);
				playerID++;
				switch (playerID) {
					case 1:
						player1 = newPlayerConnection;
					case 2:
						player2 = newPlayerConnection;
					case 3:
						player3 = newPlayerConnection;
					case 4:
						player4 = newPlayerConnection;
					case 5:
						player5 = newPlayerConnection;
					case 6:
						player6 = newPlayerConnection;
					case 7:
						player7 = newPlayerConnection;
					case 8:
						player8 = newPlayerConnection;
					case 9:
						player9 = newPlayerConnection;
					case 10:
						player10 = newPlayerConnection;
					case 11:
						player11 = newPlayerConnection;
					case 12:
						player12 = newPlayerConnection;
					case 13:
						player13 = newPlayerConnection;
					case 14:
						player14 = newPlayerConnection;
					case 15:
						player15 = newPlayerConnection;
				}
				System.out.println("Client " + (playerID) + " connected!\n");
		} catch (Exception e) {
				System.out.println("IO Exception in acceptPlayerConnections");
			}
		}
	}
	...
	public void recievePlayerNames() throws IOException {
		int nonHacker = 0;
		int host = 0;
		int tech = 0;
		int hacker = 0;
	...
	private static void recieveHostActions() throws IOException {
		...
		for (int i = 0; i < (hostInputStreamList.length); i++) {
			if (hostList[i].equals(hackedPlayers[0]) || hostList[i].equals("HACKED PLAYER")
					|| hostList[i].equals("HACKED HOST")) {
				System.out.println(hostList[i]);
				System.out.println("You were Hacked, get skipped!");
				probableHackers[i] = "HACKED PLAYER";
			} else if (hostList[i].equals(hackedPlayers[1]) || hostList[i].equals("HACKED PLAYER")
					|| hostList[i].equals("HACKED HOST")) {
				System.out.println(hostList[i]);
				System.out.println("You were Hacked, get skipped!");
				probableHackers[i] = "HACKED PLAYER";
	...
	private static void sendClientMessage() throws IOException {
		...
		for (int i = 0; i < hackerOutputStreamList.length; i++) {
			if (hackerList[i].equals("CAUGHT")) {
				hackerOutputStreamList[i]
						.writeUTF(hackedPlayers[0] + " and " + hackedPlayers[1] + " have been hacked!!");
	...
	public static void main(String[] args) throws IOException {
		...
		nonHackerList = new String[totalNumPlayers - 2];// only needs 13 b/c 2 hackers
		playerList = new String[totalNumPlayers]; // full list of players
		// socketList = new Socket[totalNumPlayers];
